...
namespace CreatureGen.Tests.Integration.Stress.Randomizers.Abilities
{
    ...
    public class PoorAbilitiesRandomizerTests : StressTests
    {
        ...
        {
            var stats = stressor.GenerateOrFail(PoorAbilitiesRandomizer.Randomize, ss => ss.Values.Any(s => s.BaseValue != 9));
            var allAbilitiesAreDefault = stats.Values.All(s => s.BaseValue == 9);
