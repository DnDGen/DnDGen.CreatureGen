...
namespace DnDGen.CreatureGen.Tests.Integration.Tables.Attacks
{
    ...
    {
        public static Dictionary<string, Dictionary<string, List<string>>> GetCreatureAttackDamageData()
        {
            var testCases = new Dictionary<string, Dictionary<string, List<string>>>();
            var attackData = AttackTestData.GetCreatureAttackData();
            ...
            //INFO: Only adding damage for original size
            //Tests will enhance natural attack dmaages for advanced sizes
            foreach (var kvp in attackData)
            {
                var creature = kvp.Key;
                var attacks = kvp.Value.Select(DataHelper.Parse<AttackDataSelection>);
                testCases[creature] = [];

                foreach (var attack in attacks)
                {
                    var key = attack.BuildDamageKey(creature, creatureData[creature].Size);
                    testCases[creature][key] = [];
                }
            }

            //TODO: Get the damage key based on the creature and attack name
            //Build damage data based on that damage key
        ...
        {
            return Infrastructure.Helpers.DataHelper.Parse(new DamageDataSelection
            {
                Roll = roll,
                Type = type,
                Condition = condition,
