...
{
    [TestFixture]
    internal class CreatureGeneratorGenerateRandomAsyncTests : CreatureGeneratorTestsBase
    {
        [TestCase(true, true)]
        [TestCase(true, false)]
        [TestCase(false, true)]
        ...
        public async Task BUG_GenerateRandomAsync_GenerateCreatureName_EmptyTemplate(string empty)
        {
            ...
            var creature = await creatureGenerator.GenerateRandomAsync(false, null, filters, empty);
            Assert.That(creature.Name, Is.EqualTo(creatureName));
            Assert.That(creature.Templates, Is.Empty);
        }
        ...
        public async Task GenerateRandomAsync_GenerateCreatureName_WithPresetNoneTemplate(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            var creature = await creatureGenerator.GenerateRandomAsync(asCharacter, null, filters, template);
            Assert.That(creature.Name, Is.EqualTo(creatureName));
        ...
        public async Task GenerateRandomAsync_GenerateCreatureName_WithMultiplePresetTemplates(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            var creature = await creatureGenerator.GenerateRandomAsync(asCharacter, null, filters, templates);
            Assert.That(creature.Name, Is.EqualTo(creatureName));
        ...
        public async Task GenerateRandomAsync_GenerateCreatureName_WithRandomTemplate_Only1TemplateCompatible(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            mockCollectionSelector
                .Setup(s => s.SelectRandomFrom(
                    It.Is<IEnumerable<string>>(cc => cc.IsEquivalentTo(creatureName, "other creature", "wrong creature", template))))
                .Returns(template);

            mockCreatureVerifier
                .Setup(v => v.GetCompatibleCreaturesForTemplate(It.IsAny<IEnumerable<string>>(), null, asCharacter, null, filters))
                .Returns((IEnumerable<string> cc, string t, bool asC, AbilityRandomizer r, Filters f) => cc.Intersect(creatures));
            mockCreatureVerifier
                .Setup(v => v.GetCompatibleCreaturesForTemplate(It.IsAny<IEnumerable<string>>(), CreatureConstants.Templates.None, asCharacter, null, filters))
                .Returns(creatures);
            mockCreatureVerifier
            ...
            var creature = await creatureGenerator.GenerateRandomAsync(asCharacter, null, filters);
            Assert.That(creature.Name, Is.EqualTo(creatureName));
            Assert.That(creature.Templates, Is.EqualTo([template]));
        }
        ...
        public async Task GenerateRandomAsync_GenerateCreatureName_WithRandomTemplate_OnlyTemplatesCompatible(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            mockCollectionSelector
                .Setup(s => s.SelectRandomFrom(
                    It.Is<IEnumerable<string>>(cc => cc.IsEquivalentTo(creatureName, "other creature", "wrong creature name", template, "other template"))))
                .Returns(template);
        ...
        public async Task GenerateRandomAsync_GenerateRandomCreatureName_WithPresetNoneTemplate(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            var creature = await creatureGenerator.GenerateRandomAsync(asCharacter, null, filters, template);
            Assert.That(creature.Name, Is.EqualTo(creatureName));
            Assert.That(creature.Templates, Is.EqualTo([CreatureConstants.Templates.None]));
        }
        ...
        public async Task GenerateRandomAsync_GenerateRandomCreatureName_WithMultiplePresetTemplates(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            var creatures = new[] { "wrong creature", creatureName, "other creature" };
            var allTemplates = new[] { "wrong template", template1, "other template", template2, "other wrong template" };
            mockCreatureVerifier.Setup(v => v.VerifyCompatibility(asCharacter, null, null, filters)).Returns(true);
        ...
        public async Task GenerateRandomAsync_ThrowException_WhenNotCompatible(bool asCharacter, string template, string type, string challengeRating, string alignment)
        {
            ...
            var expectedException = new InvalidCreatureException(null, asCharacter, null, AbilityConstants.RandomizerRolls.Default, filters, template == null ? [] : [template]);
            await Assert.ThatAsync(async () => await creatureGenerator.GenerateRandomAsync(asCharacter, null, filters, template),
                Throws.InstanceOf<InvalidCreatureException>().With.Message.EqualTo(expectedException.Message));
        }
        ...
        public async Task GenerateRandomAsync_ThrowException_WhenNotCompatible_WithAbilityRandomizer(bool asCharacter)
        {
            ...
            var expectedException = new InvalidCreatureException(null, asCharacter, null, filters, randomizer, "my template");
            await Assert.ThatAsync(async () => await creatureGenerator.GenerateRandomAsync(asCharacter, randomizer, filters, "my template"),
                Throws.InstanceOf<InvalidCreatureException>().With.Message.EqualTo(expectedException.Message));
        }
        ...
        {
            var mockTemplateApplicators = SetupCreature("creature", asCharacter, null, null, "my template");
            ...
            mockCreatureVerifier
                .Setup(a => a.GetCompatibleCreaturesForTemplate(It.IsAny<IEnumerable<string>>(), "wrong template", asCharacter, null, null))
                .Returns([]);
            ...
            var creature = await creatureGenerator.GenerateRandomAsync(asCharacter, templates: ["my template"]);
        ...
        public async Task GenerateRandomAsync_GenerateCreatureModifiedByTemplate_WithFilters(bool asCharacter)
        {
            ...
            var templateCreature = new Creature { Name = "Creature modified by template", Templates = ["my template"] };
            mockTemplateApplicators[0]
                .Setup(a => a.ApplyToAsync(It.IsAny<Creature>(), asCharacter, filters))
                .ReturnsAsync(templateCreature);

            var creature = await creatureGenerator.GenerateRandomAsync(asCharacter, null, filters, "my template");
            Assert.That(creature, Is.EqualTo(templateCreature), creature.Summary);
        }
        ...
        public async Task GenerateRandomAsync_GenerateCreatureModifiedByMultipleTemplates(bool asCharacter)
        {
            ...
            mockTemplateApplicators[1].Setup(a => a.ApplyToAsync(templateCreature1, asCharacter, null)).ReturnsAsync(templateCreature2);

            var creature = await creatureGenerator.GenerateRandomAsync(asCharacter, null, null, templates);
