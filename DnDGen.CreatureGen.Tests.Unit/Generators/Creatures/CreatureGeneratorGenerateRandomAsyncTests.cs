...
namespace DnDGen.CreatureGen.Tests.Unit.Generators.Creatures
{
    ...
    internal class CreatureGeneratorGenerateRandomAsyncTests : CreatureGeneratorTestsBase
    {
        ...
        public async Task GenerateRandomAsync_GenerateCreatureName_WithMultiplePresetTemplates(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            var filters = new Filters();
            filters.Templates.AddRange([template1, template2]);
            filters.Type = type;
            filters.ChallengeRating = cr;
            filters.Alignment = alignment;
        ...
        public async Task GenerateRandomAsync_GenerateRandomCreatureName_WithMultiplePresetTemplates(bool asCharacter, string type, string cr, string alignment)
        {
            ...
            mockCreatureVerifier
                .Setup(v => v.GetChainedTemplates(It.IsAny<IEnumerable<string>>(), filters.CleanTemplates, asCharacter, null, filters))
                .Returns((IEnumerable<string> cc, List<string> tt, bool asC, AbilityRandomizer r, Filters f) => cc.Select(c => new CreaturePrototype { Name = c }));
